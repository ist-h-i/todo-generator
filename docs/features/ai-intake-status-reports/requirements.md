...
- **In Scope**
  ...
  - Gemini-powered proposal generation, filtering, and publication into the workspace store.
...
- Backend submission transitions status to processing, records events, invokes Gemini, stores up to five proposals in processing metadata, and deletes the report after successful completion while returning the detail payload and proposals. Failures capture the error, mark the report as failed, and persist metadata for retries.【F:backend/app/services/status_reports.py†L120-L240】
