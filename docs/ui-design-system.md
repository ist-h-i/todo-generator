...
### カード（Card）
- `.surface-card` を基準に高さ可変のカードを定義。内側余白は 24px、角丸 12px、外枠は `1px solid var(--color-border-strong)`、角丸内側に `inset 0 0 0 1px color-mix(in srgb, var(--color-surface-alt) 70%, transparent)` の縁取りを追加してレイヤーを強調する。
