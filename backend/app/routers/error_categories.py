...
def list_error_categories(db: Session = Depends(get_db)) -> List[models.ErrorCategory]:
...
def delete_error_category(category_id: str, db: Session = Depends(get_db)) -> Response:
    category = db.get(models.ErrorCategory, category_id)
    if not category:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Category not found")
